<x-layout :menu="$menu">
    <x-card :title="$title">
        ...
        <form method="post" action="{{ route('ppdd2.store') }}" enctype="multipart/form-data">
            @csrf
            @include('pd/pkad/persyaratan-penyaluran/ppdd2/_form')
        </form>
        @elseif(Str::of(Route::currentRouteName())->after('.')=='edit')
        <form method="post" action="{{ route('ppdd2.update',$data->kode) }}" enctype="multipart/form-data">
            @csrf
            @method('put')
            @include('pd/pkad/persyaratan-penyaluran/ppdd2/_form')
        </form>
        ...
        @can('pengguna')
        <a href="{{ route('ppdd2.create') }}" class="btn btn-primary col-12 mt-2">Tambah</a>
        @endcan
        @endslot
                ...
                <table id="table" class="table table-responsive table-vcenter table-nowrap" width="100%">
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td>{{ $item->kode }}</td>
                            <td>{{ $item->ppdd2User->kode_kecamatan }}</td>
                            <td>{{ $item->ppdd2User->kecamatan }}</td>
                            <td>{{ $item->ppdd2User->kode_desa }}</td>
                            <td>{{ $item->ppdd2User->desa }}</td>
                            <td>
                            ...
                            @can('pengguna')
                            <td>
                                <a class="btn btn-warning col-12" href="{{ route('ppdd2.edit',$item->kode) }}">
                                    Ubah
                                </a>
                            </td>
                            <td>
                                <form method="post" action="{{ route('ppdd2.destroy', $item->kode) }}">
                                    @method('delete')
                                    @csrf
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                <div class="row text-center g-2">
                    ...
                    <div class="col-sm-12">
                        <a href="{{ asset('storage/'.$item->lorpdd3) }}" target="_blank"
                            class="card card-link card-link-pop">
                            <div class="card-body">Laporan Output Realisasi Penyerapan Dana Desa Tahap III Tahun {{
                                now()->year - 1 }}</div>
                        </a>
                    </div>
                    <div class="col-sm-12">
                        <a href="{{ asset('storage/'.$item->lorpdd1) }}" target="_blank"
                            class="card card-link card-link-pop">
                            <div class="card-body">Laporan Output Realisasi Penyerapan Dana Desa Tahap I Tahun {{
                                now()->year }}</div>
                        </a>
                    </div>
                    <div class="col-sm-12">
