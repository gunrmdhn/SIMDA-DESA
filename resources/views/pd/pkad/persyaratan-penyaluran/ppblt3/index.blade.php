<x-layout :menu="$menu">
    <x-card :title="$title">
        ...
        <form method="post" action="{{ route('ppblt3.store') }}" enctype="multipart/form-data">
            @csrf
            @include('pd/pkad/persyaratan-penyaluran/ppblt3/_form')
        </form>
        @elseif(Str::of(Route::currentRouteName())->after('.')=='edit')
        <form method="post" action="{{ route('ppblt3.update',$data->kode) }}" enctype="multipart/form-data">
            @csrf
            @method('put')
            @include('pd/pkad/persyaratan-penyaluran/ppblt3/_form')
        </form>
        ...
        @can('pengguna')
        <a href="{{ route('ppblt3.create') }}" class="btn btn-primary col-12 mt-2">Tambah</a>
        @endcan
        @endslot
                ...
                <table id="table" class="table table-responsive table-vcenter table-nowrap" width="100%">
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td>{{ $item->kode }}</td>
                            <td>{{ $item->ppblt3User->kode_kecamatan }}</td>
                            <td>{{ $item->ppblt3User->kecamatan }}</td>
                            <td>{{ $item->ppblt3User->kode_desa }}</td>
                            <td>{{ $item->ppblt3User->desa }}</td>
                            <td>
                            ...
                            @can('pengguna')
                            <td>
                                <a class="btn btn-warning col-12" href="{{ route('ppblt3.edit',$item->kode) }}">
                                    Ubah
                                </a>
                            </td>
                            <td>
                                <form method="post" action="{{ route('ppblt3.destroy', $item->kode) }}">
                                    @method('delete')
                                    @csrf
                ...
                <div class="row text-center g-2">
                    ...
                    <div class="col-sm-12">
                        <a href="{{ asset('storage/'.$item->surat_permohonan_penyaluran) }}" target="_blank"
                            class="card card-link card-link-pop">
                            <div class="card-body">Surat Permohonan Penyaluran Bulan Juli/September</div>
                        </a>
                    </div>
                    <div class="col-sm-12">
                        <a href="{{ asset('storage/'.$item->laporan_pembayaran) }}" target="_blank"
                            class="card card-link card-link-pop">
                            <div class="card-body">Laporan Pembayaran BLT Tahap 2</div>
                        </a>
                    </div>
                </div>
            </div>
