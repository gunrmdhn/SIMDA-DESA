<x-layout :menu="$menu">
    <x-card :title="$title">
        ...
        <form method="post" action="{{ route('perangkat.store') }}" enctype="multipart/form-data">
            @csrf
            @include('pd/apd/perangkat/_form')
        </form>
        @elseif(Str::of(Route::currentRouteName())->after('.')=='edit')
        <form method="post" action="{{ route('perangkat.update',$data->kode) }}" enctype="multipart/form-data">
            @csrf
            @method('put')
            @include('pd/apd/perangkat/_form')
        </form>
        ...
        @can('pengguna')
        <a href="{{ route('perangkat.create') }}" class="btn btn-primary col-12 mt-2">Tambah</a>
        @endcan
        @endslot
                ...
                <table id="table" class="table table-responsive table-vcenter table-nowrap" width="100%">
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td>{{ $item->kode }}</td>
                            <td>{{ $item->perangkatUser->kode_kecamatan }}</td>
                            <td>{{ $item->perangkatUser->kecamatan }}</td>
                            <td>{{ $item->perangkatUser->kode_desa }}</td>
                            <td>{{ $item->perangkatUser->desa }}</td>
                            <td>{{ $item->nama }}</td>
                            <td>{{ $item->jabatan }}</td>
                            <td>{{ $item->nomor_sk }}</td>
                            ...
                            @can('pengguna')
                            <td>
                                <a class="btn btn-warning col-12" href="{{ route('perangkat.edit',$item->kode) }}">
                                    Ubah
                                </a>
                            </td>
                            <td>
                                <form method="post" action="{{ route('perangkat.destroy', $item->kode) }}">
                                    @method('delete')
                                    @csrf
